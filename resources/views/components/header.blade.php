{{-- Announcement Bar - KHÔNG sticky, nằm ngoài header --}}
<div class="bg-slate-950 text-white text-xs text-center py-2.5 px-4 tracking-widest uppercase border-b border-white/5 w-full">
...
{{-- Header sticky - CHỈ chứa nav bar, không chứa announcement --}}
<header x-data="{ mobileOpen: false, searchOpen: false }" 
        class="sticky top-0 z-50 w-full bg-white border-b border-slate-200 shadow-sm">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex items-center justify-between h-16">
            ...
            <nav class="hidden md:flex items-center gap-1">
                <a href="{{ route('home') }}"
                   class="px-4 py-2 text-sm font-bold transition-all duration-200 rounded-xl {{ request()->routeIs('home') ? 'text-sky-600 bg-sky-50 border border-sky-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                    Trang chủ
                </a>
                <a href="{{ route('client.shop') }}"
                   class="px-4 py-2 text-sm font-bold transition-all duration-200 rounded-xl {{ request()->routeIs('client.shop') ? 'text-sky-600 bg-sky-50 border border-sky-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                    Sản phẩm
                </a>
                ...
                <div class="relative">
                    <a href="{{ route('client.about') }}"
                       class="block px-4 py-2 text-sm font-bold transition-all duration-200 rounded-xl {{ request()->routeIs('client.about') ? 'text-sky-600 bg-sky-50 border border-sky-100' : 'text-slate-600 hover:text-slate-900 hover:bg-slate-100' }}">
                        Giới thiệu
                    </a>
                </div>
            ...
            <div class="flex items-center gap-1 flex-shrink-0">
                ...
                <div x-show="searchOpen" x-cloak x-transition class="flex items-center gap-2">
                ...
                <div class="relative" x-data="{ open: false }" @click.outside="open = false">
                    <button @click="open = !open"
                        ...
                        <div class="w-7 h-7 bg-sky-600 rounded-xl flex items-center justify-center shadow-sm shadow-sky-600/20 flex-shrink-0">
                    ...
                    <div x-show="open" x-cloak x-transition
                         class="absolute right-0 top-full mt-2 w-48 bg-white rounded-xl shadow-xl border border-slate-100 py-2 z-50">
                        <a href="" class="flex items-center gap-3 px-4 py-2.5 text-sm font-medium text-slate-600 hover:bg-slate-50 hover:text-slate-900">
    ...
    <div x-show="mobileOpen" x-cloak
         ...
         class="md:hidden bg-white border-t border-slate-100 px-4 py-4 space-y-1">
        ...
        <a href="{{ route('client.about') }}" class="block px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50 hover:text-sky-600 rounded-xl">Giới thiệu</a>
