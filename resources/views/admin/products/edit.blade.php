                                ...
                                <div class="flex flex-wrap gap-2">
                                    @foreach($attr->values as $val)
                                        <label class="inline-flex items-center gap-1.5 bg-slate-50 border border-slate-200 px-2.5 py-1 rounded-lg text-xs cursor-pointer hover:bg-slate-100 transition">
                                            <input type="checkbox" value="{{ $val->id }}" data-name="{{ $val->value }}" data-attr="{{ $attr->name }}"
                                                   class="rounded border-slate-300 text-indigo-600 focus:ring-indigo-500 attribute-checkbox">
                                            <span>{{ $val->value }}</span>
                                        </label>
                                    @endforeach
                                </div>
                ...
                <div class="bg-white p-6 rounded-2xl shadow-sm border border-slate-200 space-y-4">
                    ...
                    <div class="space-y-4">
                        <template x-for="(variant, index) in variants" :key="variant.unique_key">
                            <div class="bg-white border border-slate-200 rounded-xl p-4 space-y-3 shadow-2xs relative">
...
function productEditForm() {
    ...
    return {
        ...
        specs: (() => {
            ...
            return (Array.isArray(rawSpecs) && rawSpecs.length > 0)
                ? rawSpecs
                : [{ key: '', value: '' }];
    ...
    return arrays.reduce((accumulator, currentArray) => {
        ...
        accumulator.forEach(accumulatorItem => {
            ...
            currentArray.forEach(currentItem => {
                ...
                result.push(
                    [].concat(accumulatorItem, currentItem)
                );
